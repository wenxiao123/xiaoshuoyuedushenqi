...
public class CategoryzyAdapter extends RecyclerView.Adapter<CategoryzyAdapter.CategoryNovelViewHolder> {
    ...
    public CategoryzyAdapter(Context mContext, List<Noval_details> novalDetails) {
    ...
    public CategoryNovelViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        return new CategoryNovelViewHolder(LayoutInflater.from(mContext)
                .inflate(R.layout.item_category_novel2, null));
    }
...
//                .into(categoryNovelViewHolder.cover);
        ImageLoaderUtils.display(mContext,categoryNovelViewHolder.cover,novalDetails.get(i).getPic(),R.drawable.cover_error,R.drawable.cover_error);
