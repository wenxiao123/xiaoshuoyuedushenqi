...
import com.novel.collection.book.animation.AnimationProvider;
import com.novel.collection.book.animation.CoverAnimation;
 ...
 */
public class Page_item extends PageView{
    ...
    public Page_item(Context context) {
        super(context);
        init(context);
    }

    public Page_item(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public Page_item(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        }
     ...
     * 处理普通翻页的事件分发
     */
    private void onVerticalTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                 y= (int) event.getY();
                 x=(int) event.getX();
...
//                cancelPage = false;
//                //是否点击了中间
//                if (downX > viewWidth / 5 && downX < viewWidth * 4 / 5){
//                   // Log.e("www3", "onCoverTouchEvent: "+2233);
//                    return true;
...
//            updateBitmap();
    ...
    private void startCoverAnim(){
        ValueAnimator va = null;
        va = ValueAnimator.ofInt(viewWidth/5, viewWidth-1);
        va.setDuration(500);
        va.setInterpolator(new AccelerateDecelerateInterpolator());
//        va.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
//            @Override
...
//        });
    ...
    public void drawBitmap2(){
        // 绘制 A 区域
        Canvas mCanvas = new Canvas(mCurPageBitmap);
        mCanvas.drawPath(getPathDefault(), mBgPaint);
        if (mType == TYPE_TXT) {
            drawText(mCanvas, mPaint,mSype);
            mHasBitmapB = mNextPosition < mContent.length();
