...
public class StatusBarUtil3 {
     ...
     * @param flag
     */
    public static void setFullScreen(Activity activity, int flag) {
        if (flag == 0) {
    ...
    public static void setFitsSystemWindows(Activity activity, boolean isFitsSystemWindows) {
        ImmersionBar.with(activity).fitsSystemWindows(isFitsSystemWindows).init();
    }
    ...
    public static void setStatusTextColor(boolean useDart, Activity activity) {
        if (activity != null) {
            ImmersionBar.with(activity).statusBarDarkFont(useDart).init();
        }
    }
    ...
    private static final String KEY_MIUI_NOVELANDROID = "ro.miui.internal.storage";
    ...
    private static void setMIUIStatusTextColor(Activity activity, boolean useDart) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            activity.getWindow().getDecorView().findViewById(android.R.id.content).setPadding(0, 0, 0, 26);
        } else {
