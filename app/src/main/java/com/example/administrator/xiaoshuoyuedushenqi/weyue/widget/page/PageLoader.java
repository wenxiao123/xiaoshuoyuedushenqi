...
import com.example.administrator.xiaoshuoyuedushenqi.entity.bean.BookRecordBean;
...
public abstract class PageLoader {
    ...
    //绘制背景
    public void setBgColor(int theme, View view) {
        ...
        if (isBookOpen) {
            ...
            //重绘
            mPageView.refreshPage();
            view.setBackgroundColor(mPageBg);
        }
    }
    ...
    //获取当前章节的章节位置
    public void setChapterPos(int position) {
        mCurChapterPos = position;
    }
...
//        canvas.drawText(time, x, y, mTipPaint);
    ...
    boolean prevChapter() {
        ...
        getDetailedChapterData(mCollBook.get_id(), Integer.parseInt(data.getWeigh()) - 1 + "", 3);
//        //加载上一章数据
...
//        else {
    ...
    boolean nextChapter() {
        if (Integer.parseInt(data.getWeigh()) + 1 >= data.getMax_num()) {
            ToastUtils.show("已经没有下一章了");
            return false;
        }
        chpter_id++;
        getDetailedChapterData(mCollBook.get_id(), chpter_id+ "", 2);
        //加载一章
...
//            //这个PageList可能为 null，可能会造成问题。
    ...
    public void getDetailedChapterData(String bookid, String id, int z) {
        Gson mGson = new Gson();
